        ...
        <div class="row align-items-start">
                ...
                <form class="form-inline" style="align-items: start;">
                    <div class="col-5">
                        <div class="form-group row columnSpacing">
                            ...
                            <div class="col-8">
                                <input type="text" readonly class="form-control-plaintext" id="patientName" v-model="patient.name" v-if="loaded" />
                            </div>
                        </div>
                        <div class="form-group row columnSpacing">
                            <label for="patientGeschlecht" class="col-4 col-form-label labelTitle">Geschlecht:</label>
                            <div class="col-8">
                                <input type="text" readonly class="form-control-plaintext" id="patientGeschlecht" v-model="patient.gender" v-if="loaded" />
                            </div>
                        </div>
                        <div class="form-group row columnSpacing">
                            <label for="patientAlter" class="col-4 col-form-label labelTitle">Alter:</label>
                            <div class="col-8">
                                <input type="text" readonly class="form-control-plaintext" id="patientAlter" v-model="patient.age" v-if="loaded" />
                            </div>
                        </div>
                        <div class="form-group row columnSpacing">
                            <label for="patientVorerkrankungen" class="col-4 col-form-label labelTitle">Vorerkrankungen:</label>
                            <div class="col-8">
                                <input type="text" readonly class="form-control-plaintext" id="patientVorerkrankungen" v-model="patient.preExistingIllness.text" v-if="loaded" />
                            </div>
                        </div>
                    </div>
            ...
            <div class="col-2">
                ...
                <div class="btn-group" role="group">
                    ...
                    <button type="button" class="btn btn-secondary" id="btn-c" @click="openABCDE(patient.status.c.notes, $event)" :class="classABCDE(patient.status.c.isSelected)" v-if="loaded">C</button>
        ...
        fillData() {
            ...
            axios(config)
                .then(function (response) {
                    ...
                    vm.patient.miscellaneaous.text =
                        response.data.data.miscellaneous;
                    vm.patient.status.a.isSelected =
                        response.data.data.AIsSelected;
                    vm.patient.status.a.notes = response.data.data.AText;
                    vm.patient.status.b.isSelected =
                        response.data.data.BIsSelected;
                    vm.patient.status.b.notes = response.data.data.BText;
                    vm.patient.status.c.isSelected =
                        response.data.data.CIsSelected;
                    vm.patient.status.c.notes = response.data.data.CText;
                    vm.patient.status.d.isSelected =
                        response.data.data.DIsSelected;
                    vm.patient.status.d.notes = response.data.data.DText;
                    vm.patient.status.e.isSelected =
                        response.data.data.EIsSelected;
        ...
        retrieveAudio() {
            ...
            axios({
                ...
                .then(function (response) {
                    ...
                    for (const audioFile of response.data.data) {
                        const blob = new Blob(audioFile, {
                            type: "audio/mpeg",
                        });
                        const url = window.URL.createObjectURL(blob);
                        // var audios = audioFile.decodeAudioData();
